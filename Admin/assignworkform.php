    ...
    if(($_REQUEST['request_id'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "") || ($_REQUEST['requestername'] == "") || ($_REQUEST['address1'] == "") || ($_REQUEST['address2'] == "") || ($_REQUEST['requestercity'] == "") || ($_REQUEST['requesterstate'] == "") || ($_REQUEST['requesterzip'] == "") || ($_REQUEST['requesteremail'] == "") || ($_REQUEST['requestermobile'] == "") || ($_REQUEST['assigntech'] == "") || ($_REQUEST['inputdate'] == "")){
     ...
     $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert">All Fields are mandatory. </div>';
        ...
        $rdate = $_REQUEST['inputdate'];     
         ...
         // Check if a technician is already assigned to this request ID
         $checkSql = "SELECT * FROM assignwork_tb WHERE request_id = '$rid'";
         $result = $conn->query($checkSql);
 
         if ($result->num_rows > 0) {
             // Technician is already assigned to this request ID
             $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert">Technician is already assigned to this Request ID.</div>';
         }
         else{
  ...
  <form action="" method="POST">
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-4">
        ...
        <input type="text" class="form-control" id="requesterzip" name="requesterzip" pattern="\d{6}" maxlength="6" value="<?php if(isset($row['requester_zip'])) { echo $row['requester_zip']; } ?>"
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-4">
        ...
        <input type="text" class="form-control" id="requestermobile" name="requestermobile" pattern="\d{10}" maxlength="10" value="<?php if(isset($row['requester_mobile'])) {echo $row['requester_mobile']; }?>"
          onkeypress="isInputNumber(event)">
      </div>
    </div>
    <div class="form-row">
